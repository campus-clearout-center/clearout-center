...
class ListItems extends React.Component {
  ...
  renderPage() {
    console.log(this.props.items.filter((item) => (item.label === 'Services')));
    return (
      <Container>
        <Header as="h2" textAlign="center" inverted>List Items</Header>
        <Card.Group>
          {this.props.items.filter((item) => (item.label === 'Services')).map((item, index) => <Items key={index} items={item}/>)}
